...
2. **Invariant (`0x0`)**: Strictly enforced.
   *FastPath check*: `region_a == region_b`

3. **Contravariant (`0x2`)**: Common for function parameters.
...
2. Resolving the variance (`0x1` for reference lifetimes, allowing reborrowing).
3. Packing this data into the FastPath word via `try_set_fast_param`.
