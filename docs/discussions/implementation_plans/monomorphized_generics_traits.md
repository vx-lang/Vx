...
1. **Entry Point Execution**: Begin type checking from the `main` function (or a specified root).
1. **Lazy Instantiation (Monomorphization)**:
...
1. **Output Generation**: Produce a pruned AST containing *only* fully concrete, monomorphized structs and functions.
