...
## 1. Custom `vx` MLIR Dialect & Real Kernel Generation (Highest Priority)

**The Gap:** Currently, `spawn on` relies on C-FFI shims to Apple CoreML/Metal.
**The Fix:**

- Implement a custom `vx` MLIR dialect (e.g., `vx.spawn`, `vx.transfer`, `vx.tensor`).
- Lower tensor math into MLIR's `linalg` and `affine` dialects rather than emitting function calls.
- Introduce hardware-specific optimization passes (loop unrolling, tiling, and vectorization) using native MLIR passes before dropping down to the LLVM dialect.
...
- Bootstrap a native Vx standard library.
