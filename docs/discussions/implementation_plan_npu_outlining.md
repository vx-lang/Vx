...
1. It extracts the region inside `vx.spawn` into a standard `func.func` (or a custom `vx.func`) placed at the module level.
1. It replaces the `vx.spawn` with a `vx.dispatch` call that references the newly created function.
1. A final `vx-to-llvm` pass simply lowers `vx.dispatch` into standard LLVM IR calls to our custom NPU runtime API (`npu_launch_kernel`).
...
1. Use MLIR's built-in `async-to-async-runtime` outlining pass, which automatically outlines `async.execute` regions into coroutine-like functions.
1. Write a custom pass to intercept the execution of these specific async functions and redirect them to the NPU driver.
...
1. Rely on the OpenMP dialect's device outlining passes.
