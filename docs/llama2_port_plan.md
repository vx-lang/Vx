...
1. **Struct Code Generation**: Fully implement MLIR codegen for struct instantiation and field accesses (e.g., `config.dim`, `state.x`).
1. **Pointers & Memory Allocation**: Implement `malloc`, `free`, and native `*mut f32` array indexing in MLIR.
1. **Math Intrinsics**: Expose `sqrtf` and `expf` via `extern` or the MLIR `math` dialect.
1. **Strings & File I/O**: Add basic support for C-style strings (`*const i8`) and FFI for `fopen`/`fread`.
...
1. **Milestone 2**: Write unit tests comparing our native Vx kernels against `llama2.c` baseline outputs for numerical precision.
1. **Milestone 4**: Execute the full `llama2.vx` binary on a small model (e.g., `stories15M.bin`) and verify coherent text generation.
