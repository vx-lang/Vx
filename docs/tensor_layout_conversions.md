...
1. **Zero-Cost**: Unlike Python, these methods are statically evaluated intrinsics. They do not execute at runtime; they instruct the compiler to output LLVM MLIR `memref.collapse_shape`, `memref.expand_shape`, and `memref.transpose` views.
1. **Safety**: Because they return new static types, subsequent type-checking ensures that the transformations align precisely with expected function signatures.
1. **Compile-time Checks**: The methods hook seamlessly into our newly built `comptime` evaluator, ensuring all shape arithmetic (e.g., verifying `128 * 128 == 4 * 64 * 64`) is mathematically sound during compilation.
