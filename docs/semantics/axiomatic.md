...
- For $x : \\text{Ref\<Tensor, Memory::NPU_HBM>}$, $\\text{Valid}(x, \\text{NPU})$ is True.
...
$$
\\frac{
{ P \\land \\text{Available}(t) } \\ \\text{try_pin}(c, t) \\ { Q }_t \\quad
{ P \\land \\neg \\text{Available}(t) } \\ \\text{fallback}(c) \\ { Q }_{Fallback}
}{
{ P } \\ \\text{match try_pin}(c, t) { \\dots } \\ { Q }\_{Host}
...
$$
\\frac{ { \\text{Valid}(x, t\_{src}) } }{ { \\text{True} } \\ \\text{transfer}(x, \\text{Memory::}t\_{dst}) \\ { \\text{Valid}(x, t\_{dst}) \\land \\neg \\text{Valid}(x, t\_{src}) }\_{Host} } \\text{ (Transfer-Move)}
$$
