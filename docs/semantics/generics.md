...
1. If $\\tau_i = T$, and $T \\in \\text{dom}(\\Delta)$, then $\\Delta(T)$ must strictly equal $\\sigma_i$.
1. If $\\tau_i = \\text{Tensor}\\langle T \\rangle$ and $\\sigma_i = \\text{Tensor}\\langle \\alpha \\rangle$, unification proceeds structurally.
...
1. **Instantiation:** A new concrete function $f'$ is forged by applying $\\Delta$ to the entire body and signature of $f$.
1. **AST Rewriting:** The generic call expression $E$ is mutated to directly call the mangled concretized function name.
...
The execution engine evaluates $\\langle S, \\sigma, \\tau \\rangle\_\\Omega$ strictly over the concretized program $P'$ where $P\_{gen} = \\emptyset$. All generic types have been successfully lowered to explicit topological memory allocations or standard host variables.
