...
- **$\\mathbb{M}\_{Host}$**: The set of Host memory locations ($Loc\_{Host}$).
- **$\\mathbb{M}\_{Topo}$**: A disjoint set of Hardware memory locations ($Loc\_{Topo}$).
...
- **$\\mathbb{V}$**: The domain of all evaluated values: $\\mathbb{V} = \\mathbb{R} \\cup \\bigcup\_{\\tau \\in \\mathcal{T}} \\mathbb{V}_{Tensor}^{\\tau} \\cup \\mathbb{V}_{Ref}$
...
- **Store ($Store$)**: Maps locations to values. Because Vx has discrete memory topologies, the store is partitioned.
  ...
  - $\\sigma = \\sigma\_{Host} \\cup \\sigma\_{Topo}$
...
The `spawn on` construct transitions the evaluation context from $\\sigma\_{Host}$ to $\\sigma\_{Topo}$.

Let $t \\in \\mathbb{T}$ be a target topology.
$\\mathcal{E} \\llbracket \\text{spawn on}(t) { S } \\rrbracket \\rho \\sigma =$
Let $\\sigma' = \\mathcal{C} \\llbracket S \\rrbracket \\rho \\sigma\_{Topo}$.
...
$\\mathcal{E} \\llbracket \\text{match } \\text{try_pin}(c, t) \\rrbracket \\rho \\sigma =$
