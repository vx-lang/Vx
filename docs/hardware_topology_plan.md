...
Akar's distinguishing feature is the elevation of distributed hardware and heterogeneous memory directly into the type system and operational semantics. We will now fully implement the **Topology and Context Engine**.
...
> Alternatively, we could start introducing a custom `akar` dialect, though that requires a heavier LLVM setup.
...
## Verification Plan
1. **Semantic Checks:** Write tests proving that Akar blocks illegal memory accesses between `Host` and `NPU`.
