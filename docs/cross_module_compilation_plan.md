...
This plan addresses how Akar will compile and link multiple files when modules are imported, ensuring that `math.add()` correctly invokes the compiled machine code for `math.ak`'s `add` function without naming collisions.

## User Review Required

> [!WARNING]
> This plan introduces **Name Mangling**. When generating LLVM MLIR, functions in modules will no longer have clean names like `@add`. They will be prefixed (e.g., `@math_ak_add`). The `main.ak` module will retain un-mangled names for ease of execution.
...
- When creating a `sub_checker` for `math.ak`, set `module_prefix = Some("math_ak_".to_string())`.
...
- When `main.ak` processes `math.add()`, it will look up `add` in `Type::Module`, retrieve the mangled name `math_ak_add`, and emit `Expr::FunctionCall("math_ak_add")`.
...
### Automated Tests
- Run `akarc tests/frontend/pass/modules_basic/main.ak --emit-mlir`. Verify that `@tests_frontend_pass_modules_basic_math_add` is generated and called correctly.
- Run `akarc tests/frontend/pass/modules_nested/main.ak --run` and verify that the JIT compiler successfully executes the cross-module code and prints the correct exit code or stdout.
