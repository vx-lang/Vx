...
- **Key Tasks**:
  ...
  - Lower Akar loops and tensor operations into MLIR's `linalg` and `affine` dialects.
...
- **Key Tasks**:
  ...
  - Integrate an LLVM ORC JIT engine into `akarc`.
  - Execute the function in memory and return the result.

## Phase 5: Runtime & FFI
Provide the data necessary for the matmul to run.
- **Objective**: Bridge Rust and Akar for input/output.
- **Key Tasks**:
  - Define the in-memory representation of an Akar `Tensor` (e.g., a struct with a data pointer and shape).
  - Write a Rust driver in `main.rs` that allocates two matrices, invokes the JIT-compiled Akar matmul via C ABI, and prints the computation time.
