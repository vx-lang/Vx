...
| `iter` | `core::iter` | 2 | partial | A16 (`zip`, `enumerate`), Vx#649 | `Iterator` with `type Item` and eleven defaults, including `fold`, `sum` and `collect`; `FromIterator`, implemented for `Vec` in `std::vec`; `Range`, `map`/`filter`/`take`/`skip`, chained to any depth; no `rev`, `product`, `min`/`max`. `sum` answers `Self::Item` (a `Default` zero and `+`) rather than Rust's caller-chosen `S : Sum`, and `FromIterator` is not a bound `collect` can state: a bound with type arguments does not parse. The adaptors name their inner item type as `I::Item`; `Map` and `Filter` take their closure as a type parameter. `std::vec`'s `VecIter`/`VecMap` implement this trait. The trait itself is `core::iter::traits`, which `std::vec` imports alone so the rest of `core::iter`'s names do not reach every program with a `Vec` (Vx#731) |
