...
| `iter` | `core::iter` | 2 | partial | A16 (`zip`, `enumerate`), Vx#649 | `Iterator` with `type Item` and eight defaults, `Range`, `map`/`filter`/`take`/`skip`, chained to any depth; no `rev`, `sum`, `fold`, `collect`. The adaptors name their inner item type as `I::Item`; `Map` and `Filter` take their closure as a type parameter. `std::vec`'s `VecIter`/`VecMap` implement this trait. The trait itself is `core::iter::traits`, which `std::vec` imports alone so the rest of `core::iter`'s names do not reach every program with a `Vec` (Vx#731) |
