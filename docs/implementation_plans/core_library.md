...
| `option` | `core::option` | 1 | partial | Vx#526 for `expect`, Vx#711 for `inspect` | the combinators plus `is_none_or`, `or_else`, `map_or_else`, `take`, `replace`; `ok_or`/`ok_or_else` live in `core::result` to avoid an import cycle; `zip`; no `flatten`, `unwrap_or_default` (`T::default()` on a bounded parameter is not resolved), or the reference-returning methods |
...
| `iter` | `core::iter` | 2 | partial | Vx#649 | `Iterator` with `type Item` and thirty-nine defaults, including `fold`, `sum`, `product`, `collect`, `reduce` and `max`/`min` with their `_by` and `_by_key` forms, and the adaptor methods `map`, `filter`, `take`, `skip`, `step_by`, `chain`, `take_while`, `skip_while`, `map_while`, `inspect`, `scan`, `fuse`, `peekable` (with `peek` and `next_if`), `flat_map`, `flatten`, `rev`, `zip` and `enumerate` (in `core::iter::adapters`); `FromIterator`, `Sum` and `Product`, the last two for every number type; `Range`. `Peekable` and `FlatMap` carry the item they hold as an extra type parameter, since a field cannot name `I::Item`, and `flat_map`'s closure answers an iterator rather than an `IntoIterator`; `DoubleEndedIterator` with `rfold`, `rfind` and `nth_back`, implemented for `Range`, `Map`, `Filter`, `Chain`, `Inspect`, `Rev`, `Take`, `Skip`, `StepBy`, `Enumerate`, `Zip` and `Vec`'s iterator; `ExactSizeIterator` with `len` and `is_empty`, for the same types but `Filter` and `Chain`, and for `Fuse` and `Peekable`; `partition` and `unzip`, which fill two collections through `Extend`, implemented for `Vec` along with `Default`. `sum` and `product` answer `Self::Item` rather than Rust's caller-chosen `S : Sum`, and `FromIterator` is not a bound `collect` can state: a bound with type arguments does not parse. The adaptors name their inner item type as `I::Item`; `Map` and `Filter` take their closure as a type parameter. `std::vec`'s `VecIter`/`VecMap` implement this trait. The trait itself is `core::iter::traits`, which `std::vec` imports alone so the rest of `core::iter`'s names do not reach every program with a `Vec` (Vx#731) |
