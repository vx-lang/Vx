...
| `iter` | `core::iter` | 2 | partial | A16 (`zip`, `enumerate`), Vx#649 | `Iterator` with `type Item` and twenty-nine defaults, including `fold`, `sum`, `product`, `collect`, `reduce` and `max`/`min` with their `_by` and `_by_key` forms, and the adaptor methods `map`, `filter`, `take`, `skip`, `step_by`, `chain`, `take_while`, `skip_while`, `map_while` and `inspect` (in `core::iter::adapters`, whose names reach every program with a `Vec` until Vx#731); `FromIterator`, `Sum` and `Product`, the last two for every number type; `Range`; no `rev`, `zip`, `enumerate` (tuples, Vx#533). `sum` and `product` answer `Self::Item` rather than Rust's caller-chosen `S : Sum`, and `FromIterator` is not a bound `collect` can state: a bound with type arguments does not parse. The adaptors name their inner item type as `I::Item`; `Map` and `Filter` take their closure as a type parameter. `std::vec`'s `VecIter`/`VecMap` implement this trait. The trait itself is `core::iter::traits`, which `std::vec` imports alone so the rest of `core::iter`'s names do not reach every program with a `Vec` (Vx#731) |
