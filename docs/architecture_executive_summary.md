...
1. **`LOCAL_HIR_STREAM`: `Vec<HirInstruction>`** (Dense instruction array with `u32` indices into the Type Stream)
...
1. **`LocalWorkerState` (The Phase Context):** Private, fully mutable context per thread.
