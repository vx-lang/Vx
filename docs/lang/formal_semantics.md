...
$$
\frac{
    \Gamma, \Delta \vdash e : \text{Ref}\langle T, M_{src} \rangle \quad M_{tgt} \in \text{MemorySpaces}
}{
    \Gamma, \Delta \vdash \text{transfer}(e, M_{tgt}) : \text{Ref}\langle T, M_{tgt} \rangle
}
\quad (\text{T-TRANSFER})
$$
...
\frac{
    ...
    \Gamma, \Delta \vdash e.\text{try\_pin}(\tau) : \text{HardwareState}\langle T, \tau \rangle
}
\quad (\text{T-TRY-PIN})
$$
...
\frac{
    ...
    \langle \text{Verified}(v).\text{try\_pin}(\tau), \mu, S \rangle \longrightarrow \langle \text{Available}(\text{Pinned}(v, \tau)), \mu, \text{Acquire}(\tau, S) \rangle
}
...
\frac{
    ...
    \langle \text{Verified}(v).\text{try\_pin}(\tau), \mu, S \rangle \longrightarrow \langle \text{Saturated}(\text{Verified}(v)), \mu, S \rangle
}
\quad (\text{E-PIN-FAIL})
$$
