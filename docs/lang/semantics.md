...
2. The runtime reserves resources on $\tau$.
3. Execution of the block $B$ is enqueued on $\tau$.
4. The spawning unit $\rho$ continues asynchronously unless a data dependency explicitly synchronizes the contexts.
5. All variables captured within $B$ that are not in a shared or transferred memory space relative to $\tau$ will result in a compiler error.
...
2. Evaluates to a target reference $T$ in space $M_2$.
3. The runtime schedules an asynchronous or synchronous DMA transfer from $M_1$ to $M_2$.
4. Linearity: If $S$ represents uniquely owned data, the `transfer` consumes $S$. Future use of $S$ in space $M_1$ is invalid.
