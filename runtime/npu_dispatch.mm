...
// Fused attention on the Neural Engine, as one CoreML operation.
//
// `scaled_dot_product_attention` rather than a matmul/softmax/matmul chain:
// fused attention exists so the score matrix is never materialised, and
// decomposing it into GEMM calls defeats it (Vx#319). The model is looked up by
// shape exactly as the matmul primitives are, so which attentions have a
// primitive is decided by the build list (Vx#173).
//
// f16 only, and only above roughly 2M elements per operand. CoreML answers CPU
// for every fp32 attention and for small f16 ones, so a primitive outside that
...
extern "C" int vx_dispatch_ane_attention(void *o, const void *q, const void *k,
  ...
  @autoreleasepool {
    ...
    NSString *name = [NSString
        stringWithFormat:@"attention_%lldx%lldx%lld_fp16.mlmodelc", sq, sk, hd];
    MLModel *model = models[name];
    if (!model) {
      MLModelConfiguration *config = [[MLModelConfiguration alloc] init];
      config.computeUnits = MLComputeUnitsAll;
      model = [MLModel modelWithContentsOfURL:[NSURL fileURLWithPath:name]
    ...
    // CoreML's interface is fp32 even when the compute is fp16, and the op
    // needs rank 3, so the rank-2 operands are presented as (1, seq, hd).
    const size_t q_elems = (size_t)sq * (size_t)hd;
    const size_t kv_elems = (size_t)sk * (size_t)hd;
    const __fp16 *q16 = (const __fp16 *)q;
    const __fp16 *k16 = (const __fp16 *)k;
    ...
    id<MLFeatureProvider> inputs = [[MLDictionaryFeatureProvider alloc]
        initWithDictionary:@{@"q" : aq, @"k" : ak, @"v" : av}
                     error:&error];
    id<MLFeatureProvider> outputs = [model predictionFromFeatures:inputs
                                                            error:&error];
    if (error || !outputs) {
      VX_NPU_LOG("[Vx Dispatcher] attention prediction failed; using the CPU "
                 "shim\n");
      return 0;
    }

    NSString *outName = [outputs.featureNames anyObject];
    MLMultiArray *outArray =
        [outputs featureValueForName:outName].multiArrayValue;
    ...
    VX_NPU_LOG("--- COREML RAN THE fp16 %lldx%lldx%lld ATTENTION ---\n", sq, sk,
               hd);
    return 1;
  }
}
...
extern "C" uint64_t vx_plugin_dispatch_async(const void *binary_payload,
  ...
  // A fused attention the compiler classified (Vx#378). Tried before the GEMM
  // routes because attention is not a GEMM and must not be decomposed into one.
  vx_attention_plan attn;
  if (vx_attention_plan_decode(binary_payload, payload_size, device_args,
                               arg_tags, num_args, &attn) &&
      attn.dtype == VX_DTYPE_F16) {
    VX_NPU_LOG("[Vx Dispatcher] Recognised attention %lldx%lldx%lld f16\n",
