...
| Feature / Dimension | PyTorch | JAX + XLA | Mojo | Vx |
| --- | --- | --- | --- | --- |
...
A `.vx` script can be run using the built-in JIT compilation engine:
...
To run the full suite across heterogeneous topologies:
...
The project is actively building out the language features, the type-checker, and the MLIR optimization pipeline. Please check out the [ROADMAP.md](./ROADMAP.md) for a detailed list of completed and upcoming milestones!
